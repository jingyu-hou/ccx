...
!
     ...
     &  iuel,iprestr,nstam,ndamp,nmpmat,phase_inf)
!
...
!
     ...
     &  iprestr,nstam,ier,ndamp,nmpmat,nmpmatl
!
      real*8 temperature,tempact,xfreq,tpinc,tpmin,tpmax
!
      parameter(nentries=17)
      integer phaseinf(7) ,maxv1(3),maxv2(7),phase_inf(4),nwz1
C phaseinf:1存放相的数量,2存放相cct/ttt曲线数量（比如：有1%,50%则曲线数量为2）
C 3存放ttt/cct曲线中点的最大数量,4存放孕育期曲线点的最大数量和存放相平衡点的最大数量
C 5存放相热物属性点的最大数量,6不同冷速下转变量与温度关系点的最大数量,7转变量-温度关系中有几个冷速
C phase_inf:1相的数量;2曲线条数;3曲线点的最大数量;4有几条转变量-温度关系曲线
       maxv2=0
       phase_inf=0;phaseinf=0
!
...
!
     ...
     &          (textpart(1)(1:15).eq.'*LIQUIDFRACTION').or.
...
!
     ...
     &           ipoinp,inp,ipoinpc)
         elseif(textpart(1)(1:12).eq.'*METALPOWDER') then
            ...
            if (ncmat .gt. 2) then
               write(*,*)
     &             'isotropic elasticity should be defined'
               write(*,*) '       '
               call inputerror(inpc,ipoinpc,iline,
     &              "*ELASTIC%",ier)
            elseif (ncmat .lt. 2) then
               write(*,*)
     &             '*METAL POWDER should be defined after *ELASTIC'
               write(*,*) '       '
               call inputerror(inpc,ipoinpc,iline,
     &              "*ELASTIC%",ier)
            endif
...
!
     ...
     &              ipoinp,inp,ipoinpc)
               if((istat.lt.0).or.(key.eq.1)) exit
            enddo
         elseif(textpart(1)(1:12).eq.'*PHASECURVE') then
                maxv1=0
            do
              call getnewline(inpc,textpart,istat,n,key,iline,ipol,inl,
     &                ipoinp,inp,ipoinpc)
              if((istat.lt.0).or.(key.eq.1)) exit
               do i=1,3
                   if(i.gt.3)exit
                read(textpart(i)(1:10),'(i10)',iostat=istat)phaseinf(i)
                 if(phaseinf(i).lt.0)then
                   write(*,*) '*ERROR:  phase change information input'
                   return
                 endif
               enddo
                maxv1(1)=max(maxv1(1),phaseinf(1))
                maxv1(2)=max(maxv1(2),phaseinf(2))
                maxv1(3)=max(maxv1(3),phaseinf(3))
                if(phaseinf(3).gt.0)then
                  do nwz1=1,phaseinf(3)
                   call getnewline(inpc,textpart,istat,n,key,iline,ipol,
     &                           inl,ipoinp,inp,ipoinpc)
                  enddo
                endif
            enddo
            phaseinf(1)=max(maxv1(1),maxv2(1))
            phaseinf(2)=max(maxv1(2),maxv2(2))
            phaseinf(3)=max(maxv1(3),maxv2(3))
C 考虑塑性应变时需要将前面14个状态变量留空
C            nstate=max(0,14+phaseinf(1))

            maxv2(1)=maxv1(1)
            maxv2(2)=maxv1(2)
            maxv2(3)=maxv1(3)
         elseif(textpart(1)(1:17).eq.'*PHASEEQUILIBRIUM'.or.
     &    textpart(1)(1:17).eq.'*INCUBATIONPERIOD' ) then
                maxv1(1)=0
            do
              call getnewline(inpc,textpart,istat,n,key,iline,ipol,inl,
     &                ipoinp,inp,ipoinpc)
              if((istat.lt.0).or.(key.eq.1)) exit
                read(textpart(2)(1:10),'(i10)',iostat=istat)phaseinf(4)
                maxv1(1)=max(maxv1(1),phaseinf(4))
                do nwz1=1,phaseinf(4)
                 call getnewline(inpc,textpart,istat,n,key,iline,ipol,
     &                           inl,ipoinp,inp,ipoinpc)
                enddo
            enddo
            phaseinf(4)=max(maxv1(1),maxv2(4))
            maxv2(4)=maxv1(1)
     ...
     &                ipoinp,inp,ipoinpc)
              if((istat.lt.0).or.(key.eq.1)) exit
                read(textpart(2)(1:10),'(i10)',iostat=istat)phaseinf(5)
                maxv1(1)=max(maxv1(1),phaseinf(5))
                do nwz1=1,phaseinf(5)
                 call getnewline(inpc,textpart,istat,n,key,iline,ipol,
     &                           inl,ipoinp,inp,ipoinpc)
                enddo
            enddo
            phaseinf(5)=max(maxv1(1),maxv2(5))
            maxv2(5)=maxv1(1)
     ...
     &                ipoinp,inp,ipoinpc)
              if((istat.lt.0).or.(key.eq.1)) exit
                read(textpart(2)(1:10),'(i10)',iostat=istat)phaseinf(7)
                maxv1(2)=max(maxv1(2),phaseinf(7))
                read(textpart(3)(1:10),'(i10)',iostat=istat)phaseinf(6)
                maxv1(1)=max(maxv1(1),phaseinf(6))
                do nwz1=1,phaseinf(6)
                 call getnewline(inpc,textpart,istat,n,key,iline,ipol,
     &                           inl,ipoinp,inp,ipoinpc)
                enddo
                maxv1(2)=maxv1(2)+1
            enddo
             phaseinf(6)=max(maxv1(1),maxv2(6))
             phaseinf(7)=max(maxv1(2),maxv2(7))
             maxv2(6)=maxv1(1)
             maxv2(7)=maxv1(2)
         elseif(textpart(1)(1:16).eq.'*PHASELATENTHEAT'.or.
     ...
     &              ipoinp,inp,ipoinpc)
               if((istat.lt.0).or.(key.eq.1)) exit
            enddo
         elseif(textpart(1)(1:11).eq.'*PHASECTROL'.or.
     &          textpart(1)(1:8).eq.'*PHASEGS'.or.
     &          textpart(1)(1:8).eq.'*PHASEYS') then
            call getnewline(inpc,textpart,istat,n,key,iline,ipol,inl,
     &           ipoinp,inp,ipoinpc)
...
!
     ...
     &           ipoinp,inp,ipoinpc)
      ...
      enddo loop
      phase_inf(1)=phaseinf(1)
      phase_inf(2)=phaseinf(2)
      phase_inf(3)=maxval(phaseinf(3:6))
      phase_inf(4)=phaseinf(7)
      if(phaseinf(1).gt.0)then
         if(nstate.ge.13)then
            nstate=max(nstate,14+max(phaseinf(1)+3,
     &           2*phaseinf(1)+2))
         else
            nstate=max(nstate,max(phaseinf(1)+3,2*phaseinf(1)))
         endif
      endif
!
      do i=1,nset
