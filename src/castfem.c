...
void castfem(double **cop,ITG *nk,ITG **ipkonp,ITG **konp,char **lakonp,
		  ...
		  double *qfx,ITG *nflcon,double *flcon,double *lh,ITG *nmat){
  ...
  ITG *ipointer=NULL,*mast1=NULL,*irowt=NULL,*irowv=NULL,*irowp=NULL,
    ...
    *inomast=NULL,*ielslav=NULL,*ielmast=NULL,*nr=NULL,*inomat=NULL,
    ...
    ithermalref,nrhs=1,ipower=1,icounter=1,
    *nhtcon=NULL,*nsrhocp=NULL,imat=0,n_nyscon=101,icastpost=0,
    singularflag;
  ...
  double *yy=NULL,*xsolidsurf=NULL,*dt=NULL,*vcon=NULL,*x=NULL,
    ...
    *htcon=NULL,*srhocp=NULL,*gradtempi=NULL,*gradtempn=NULL,*nyscon=NULL,
    *coolingrate=NULL,*tempold=NULL,*gtemp=NULL,fl_ny=0.1,
  ...
  printf(" Using up to %d cpu(s) for CAST.\n", num_cpus);
  ...
  FORTRAN(htchart,(nrhcon,rhcon,nshcon,shcon,nflcon,flcon,
          nsrhocp,srhocp,nhtcon,htcon,lh,nmat,physcon,ntmat_,ithermal));
  /*for(imat=0;imat<202;imat++){
    printf("htcon data: i=%d , data=%f\n",imat,htcon[imat]);
  }*/

  /* preparing postprocessing variables for casting */
  FORTRAN(templiquidussolidus,(nflcon,flcon,&templiq,&tempsol,
          &icastpost,ntmat_));
  if(icastpost==1){
    NNEW(solstart,double,*nk);
    NNEW(solend,double,*nk);

    NNEW(coolingrate,double,*nk);
    NNEW(tempold,double,*nk);
    NNEW(gtemp,double,*nk);

    for(i=0;i<*nk;i++){solstart[i]=0; solend[i]=0; coolingrate[i]=-1; gtemp[i]=0;}
  }
  ...
  FORTRAN(initial_cast,(yy,nk,co,ne,ipkon,kon,lakon,x,y,z,xo,yo,zo,nx,ny,nz,
			 isolidsurf,neighsolidsurf,xsolidsurf,dh,nshcon,shcon,nrhcon,rhcon,
			 nflcon,flcon,nsrhocp,srhocp,lh,vold,vcon,ntmat_,iponoel,inoel,
...
#ifdef SPOOLES
      spooles_factor(adbp,aubp,adb,aub,&sigma,icolp,irowp,&neqp,&nzsp,
		     &symmetryflag,&inputformat,&nzsp,&singularflag);
#else
  ...
  if(compressible==0){
    FORTRAN(applyboun_cast,(nodeboun,ndirboun,nboun,xbounact,ithermal,nk,iponoel,
			  inoel,vold,vcontu,t1act,isolidsurf,nsolidsurf,
			  xsolidsurf,nfreestream,ifreestream,iturbulent,vcon,
			  shcon,nshcon,rhcon,nrhcon,flcon,nflcon,srhocp,nsrhocp,lh,ielmat,ntmat_,
			  physcon,v,&compressible,&ismooth,nmpc,nodempc,ipompc,coefmpc,
  ...
  do{
    ...
    for(i=0; i<num_cpus; i++)  {
      ithread[i]=i;
      pthread_create(&tid[i], NULL, (void *)mafillv1rhscastmt, (void *)&ithread[i]);
    }
    ...
    for(i=0; i<num_cpus; i++)  {
      ithread[i]=i;
      pthread_create(&tid[i], NULL, (void *)mafillprhscastmt, (void *)&ithread[i]);
    }
    for(i=0; i<num_cpus; i++)  pthread_join(tid[i], NULL);
    ...
    for(i=0; i<num_cpus; i++)  {
      ithread[i]=i;
      pthread_create(&tid[i], NULL, (void *)mafillv2rhscastmt, (void *)&ithread[i]);
    }
    for(i=0; i<num_cpus; i++)  pthread_join(tid[i], NULL);
    ...
    if(*ithermal>1){
      ...
      for(i=0; i<num_cpus; i++)  {
	ithread[i]=i;
	pthread_create(&tid[i], NULL, (void *)mafilltrhscastmt, (void *)&ithread[i]);
      }
      for(i=0; i<num_cpus; i++)  pthread_join(tid[i], NULL);
    ...
    if(*iturbulent!=0){
      ...
      for(i=0; i<num_cpus; i++)  {
	ithread[i]=i;
	pthread_create(&tid[i], NULL, (void *)mafillkrhscastmt, (void *)&ithread[i]);
      }
      for(i=0; i<num_cpus; i++)  pthread_join(tid[i], NULL);
    ...
    FORTRAN(con2phys_cast,(vold,vcon,v,nk,
		      ielmat,ntmat_,shcon,nshcon,rhcon,nrhcon,
          flcon,nflcon,htcon,nhtcon,srhocp,nsrhocp,lh,&iout,
		      nmethod,&iconvergence,physcon,iponoel,inoel,ithermal,
		      nactdoh,&iit,&compressible,&ismooth,vcontu,vtu,iturbulent,
		      inomat,nodeboun,ndirboun,nboun,mi,co,&factor,tempold));
    ...
    if(icastpost==1){
      ...
      //gradtempn:grad(T) on nodes
        ...
        FORTRAN(calcgradtemp_cast,(kon,lakon,ipkon,
                mi,ielmat,ipvar,var,vold,ne,gradtempi));
        ...
        FORTRAN(preparenycalc,(nk,vold,tempold,mi,coolingrate,gtemp,
                gradtempn,&dtimef,&fl_ny,flcon,nflcon,ntmat_,ithermal));
        //FORTRAN(test_tdot_and_g,(nk,vold,tempold,mi,coolingrate,gtemp,
        //        gradtempn,&dtimef,&fl_ny,flcon,nflcon,ntmat_,ithermal));
	...
	FORTRAN(calcmach,(vold,vcon,v,nk,
      ...
      FORTRAN(frd_cast,(&kode,&timef,nk,mi,inomat,vold,flcon,nflcon,
         ntmat_,ithermal,istep,iinc,&icounter));
      //FORTRAN(test_cast,(&kode,&timef,nk,mi,inomat,vold,coolingrate,gtemp,
      //   ntmat_,ithermal,istep,iinc,&icounter));
    ...
    if((iit==jmax[1])||(iconvergence==1)){

      if(icastpost==1){
        NNEW(nyscon,double,2*n_nyscon);
        FORTRAN(nystarflcrchart,(&n_nyscon,nyscon,nflcon,flcon,ntmat_));

        FORTRAN(postprocess_cast,(&kode,&timef,nk,mi,inomat,&n_nyscon,nyscon,
                coolingrate,gtemp,solstart,solend,istep,iinc,&icounter));
  ...
  SFREE(htcon);SFREE(nhtcon);SFREE(nsrhocp);SFREE(srhocp);
  SFREE(coolingrate);SFREE(tempold);SFREE(gtemp);
