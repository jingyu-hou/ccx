...
!
      ...
      elseif((iperturb.eq.1).and.(istep.gt.1)) then
         write(*,*) '*ERROR reading *CAST: perturbation analysis is'
...
!
      if(istep.lt.1) then
         write(*,*) '*ERROR reading *CAST: *HEAT TRANSFER can only '
...
!
         ...
         elseif(textpart(i)(1:7).eq.'SIMPLEC') then
            ...
            write(*,*) 
     &        '*WARNING reading *CAST: parameter not recognized:'
            write(*,*) '         ',
...
!
      if((ithermal.eq.0).and.(iexpl.eq.1)) then
         write(*,*) '*ERROR reading *CAST: please define initial '
...
!
      ...
      elseif(solver(1:7).eq.'PARDISO') then
         ...
         write(*,*) '*WARNING reading *CAST: unknown solver;'
         write(*,*) '         the default solver is used'
      endif
!
!     To specify the solver as cast one in nonlingeo.c
!     Cast sover is based on FEM-CFD method
!     PS:
!     physcon(9) without operation here: FVM solver for pure CFD
!     physcon(9)=physcon(9)+10.d0 :FEM solver for pure CFD
!
      physcon(9)=physcon(9)+20.d0
 !
...
!           transient casts
!
            write(*,*) '*WARNING reading *CAST: a nonlinear geometric
...
!
!           steady state casts: initial increment time and step
...
!
      if(tinc.le.0.d0) then
         write(*,*) '*ERROR reading *CAST: initial increment size is 
     &negative'
      endif
      if(tper.le.0.d0) then
         write(*,*) '*ERROR reading *CAST: step size is negative'
      endif
      if(tinc.gt.tper) then
         write(*,*) '*ERROR reading *CAST: initial increment size exc
     &eeds step size'
      endif
!      
      if(idrct.ne.1) then
...
!
     ...
     &     ipoinp,inp,ipoinpc)
      if((istat.lt.0).or.(key.eq.1)) then
         write(*,*) '*ERROR reading *CAST: latant heat'
...
!
      read(textpart(1)(1:20),'(f20.0)',iostat=istat) lh
      if(istat.gt.0) then
         call inputerror(inpc,ipoinpc,iline,
     &        "*CAST%",ier)
         return
