...
//        printf("\n qa1 is nan %e \n", qa[1]);
    ...
    if(qa[2]>0.){
	idivergence=1;
//        printf("\n idivergence 103 qa2 %e \n", qa[2]);
	}
    if(*singularflag!=0){
	idivergence=1;
//        printf("\n idivergence 107 singularfalg\n");
	}
...
//	    (((ram[1]*cam[1]<c2[1]*uam[1]*ram2[1])||(ram[1]<=ral*qam[1])||
		...
		/*printf("check convergency:%d,%d,%d,%d,%d,%d,%d\n",
		ram[1]<=c1[1]*qam[1],
		cam[2]<*deltmx,
		cam[1]<=c2[1]*uam[1],
		...
		qa[1]<=ea*qam[1],
		cam[1]<1.e-8);
		printf("ram[1]=%E, qam[1]=%E, c1[1]=%f, ral=%f\n",ram[1],qam[1],c1[1],ral);*/
    }
	//if(cam[4]<0.05) {iconvergence=1;}
    /* thermomechanical */
...
//    MPADD start
	...
	*r_abs=energy[0]+energy[1]+energy[2]+energy[3]-*energyref-*allwk-*dampwk;
	...
	if(fabs(*r_abs)>=*enetoll/4) {
	    ...
	    r_rel=*r_abs/(*emax);
	    if(r_rel<=-maxdecay) {
		   idivergence=1;printf("checkconverge 222\n");
	    }else{
...
// # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
	...
	FORTRAN(writesta,(istep,iinc,icutb,iit,ttime,time,dtime,inext));
	if(*uncoupled){
	    if(*ithermal==2){
	        *iitterm=*iit;
		*ithermal=1;
	...
	*theta=*theta+*dtheta;
	...
	if(((*iit>ic)&&(*mortar==0))||((*mortar>1)&&(*iit>200))){
	    ...
	    FORTRAN(writestadiv,(istep,iinc,icutb,iit,ttime,time,dtime,inext));
	...
	/* check for diverging residuals */
	 ...
	 cf. WeICME.c); therefore, a comparison of cam[2]
...
//		    printf("\n idivergence 521 ram0 %e \n", ram[0]);
		...
		if(ram[0]>1.e9){
		    printf("divergence allowed: residual force too large %e\n",ram[0]);
		    if((ram1[0]>ram2[0])&&(ram[0]>ram2[0])&&(ram[0]>c1[0]*qam[0])){
			idivergence=1;
//			printf("\n idivergence 531 ram0 %e \n", ram[0]);
		    }
		}
...
//			    printf("\n idivergence 555 ram0 %e \n", ram[0]);
	    ...
	    if((idivergence==0)&&((*nmethod==-1)&&(qa[3]>cetol))){
		idivergence=2;
//		printf("\n idivergence 566 qa3 %e \n", qa[3]);
	    }
			...
			idivergence=1;
//			printf("\n idivergence 576 ram1 %e \n", ram[1]);
//          printf("ram[1]=%f, ram1[1]=%f, ram2[1]=%f, c1[1]=%f, qam[1]=%f\n",ram[1],ram1[1],ram2[1],c1[1],qam[1]);
		    }
		...
		if((cam[2]<1.e30)&&(cam[2]>*deltmx)) {
		    idivergence=2;
//		    printf("\n idivergence 586 cam2 %e \n", cam[2]);
		}
	    }
		...
		if(*idrct==1){
		    ...
		    *istab=0;
		    if((qa[2]<=0.)&&(idivergence==1)&&(*mortar==1)&&(*icutb==0)){
		    }else{
		      if(*itp==1){
		         *itp=0;
		         (*inext)--;
	...
	if(*iit>=ir){
	    ...
	    if((((iest>ic)||(*iit==ic))&&(*mortar!=1))||((*mortar==1)&&(*iit==20))){
