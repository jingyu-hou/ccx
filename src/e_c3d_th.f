...
!
     ...
     &  ntmat_,t0,t1,ithermal,vold,vini,iperturb,nelemload,
     ...
     &  network,ipobody,xbody,ibody,rdpcon,gscon)
!
...
!
     ...
     &  ii,jj,id,ipointer,ig,kk,kkk,istiff,iperm(20),ipompc(*),mi(*),
...
!
     ...
     &  xstateini(nstate_,mi(1),*),heatnod,heatfac,vini(0:mi(2),*),
     &  rdpcon(10,3,*),gscon(30,5,*),plratio,flux
!
...
!            mint2d=1
!         elseif((lakonl(4:4).eq.'8').or.(lakonl(4:6).eq.'20R')) then
         if((lakonl(4:4).eq.'8').or.(lakonl(4:6).eq.'20R')) then
            if((lakonl(7:7).eq.'A').or.(lakonl(7:7).eq.'E')) then
               mint2d=2
            else
               mint2d=4
     ...
     &           nshcon,rhcon,nrhcon)
      ...
      do i=1,nope
         if(ithermal(2).ne.2) then
            do j=1,3
               xl(j,i)=co(j,konl(i))+vini(j,konl(i))
            enddo
         endif
      enddo
...
!            do i1=1,8
!               write(*,*) co(1:3,konl(i1)),vold(1:3,konl(i1))
!            enddo
            write(*,*) 
            xsj=dabs(xsj)
            nmethod=0
...
!
     ...
     &       nshcon,sph,xstiff,kkk,nelem,istiff,mi(1),xstateini,nstate_,
     &       nelcon)
!
...
!
!           plastic work heat flux
!
            plratio = rdpcon(10,1,imat)
            if((plratio.lt.-1.d-6).and.(dtime.gt.0.d0)) then
               plratio = -plratio
               do j=1,3
                  pgauss(j)=0.d0
                  do i1=1,nope
                  ...
                  enddo
               enddo
               flux=plratio*xstateini(10,kk,nelem)/dtime
               do jj=1,nope
                  ff(jj)=ff(jj)+flux*shpj(4,jj)*weight
               enddo
            endif
...
!     &            ((lakonl(4:4).eq.'6').and.(nopes.eq.4))) then
             if((lakonl(4:4).eq.'6').and.(nopes.eq.4)) then
                xi=gauss2d1(1,i)
                et=gauss2d1(2,i)
                weight=weight2d1(i)
