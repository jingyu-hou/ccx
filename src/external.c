...
static void getExternalBehaviours(WeICMEExternalBehaviour*** p,
				  size_t **s)
{
  static WeICMEExternalBehaviour* rf = NULL;
  static size_t nrf = 0;
  *p = &rf;
  *s = &nrf;
}
...
static void treatExternalBehaviour(const char *n,
  ...
  if(WeICME_searchExternalBehaviour(n)==NULL){
#ifdef WeICME_EXTERNAL_BEHAVIOUR_DEBUG
    fprintf(stdout,"treatExternalBehaviour: registring material (library '%s', function '%s')\n",l,f);
#endif
    RENEW(*rf,WeICMEExternalBehaviour,*nrf+1);
...
WeICME_searchExternalBehaviour(const char* n){
  WeICMEExternalBehaviour** rf;
  size_t *nrf;
  getExternalBehaviours(&rf,&nrf);
  size_t i = 0;
...
void WeICME_registerExternalBehaviour(const char * n){
  ...
  const char * f;
  // interface name
  WeICMEInterface i = WeICME_STANDARD_INTERFACE;
  // current position
  const char * c = n+1;
  ...
  if((s>=10)&&(strncmp(c,"ABAQUSNL_",9)==0)){
    i = WeICME_ABAQUSNL_INTERFACE;
    c += 9;
  } else if((s>=8)&&(strncmp(c,"ABAQUS_",7)==0)){
    i = WeICME_ABAQUS_INTERFACE;
    c += 7;
  }
  p1 = c;
  p2 = c = search(c,e,'_');
