...
!
     ...
     &     kspt,kstep,kinc,iloop,nmethod,user_hardening,user_creep
      ...
      integer xiso_counter, yiso_counter
!
     ...
     &  dtemp,pnewdt, mstress, mbstress, cb, devbeta(6),params(8)
!
...
c     &  2,3,2,3/
!     xstateini = 1/2 ( I - C^p ) which is equal to negative E^p

!      open(1110, file='metalpowder.txt')
!      if ((iel .eq. 1) .and. (iint .eq. 1)) then 
!        write(1110,*) "time=",time
!        write(1110,*) "S"
!        write(1110,"(6E10.3)") (stre(i),i=1,6)
!        write(1110,*) "E0"
!        write(1110,"(6E10.3)") (emec0(i),i=1,6)
!        write(1110,*) " " 
!      endif
...
c     if ( iel == 1 .and. iint == 1 ) then
c        write ( 399, '(A)' ) "deformation info"
c        write ( 399, 20 ) emec(1:6)
c     end if


c     open ( unit = 1300, file = 'debug1300.txt', position = 'append' )
c     write ( 1300, '(2I6,13E13.4)' )
c    &     iel, iint, xstateini(1:nstate_,iint, iel)
c     close ( 1300 )
...
c     if ( iel == 1 .and. iint == 1 ) then
c        write ( 399, * ) xstateini(2:7, iint, iel)
c        write ( 399, * ) cpl(1:6)
c     end if
      ...
      ee=elconloc(1)
      un=elconloc(2)
      um=ee/(1.d0+un)                  ! 2 times shear modulus
      al=um*un/(1.d0-2.d0*un)          ! lame's first parameter
      xk=al+um/3.d0                    ! bulk modulus
...
c     if ( iel == 20 .and. iint == 1 ) then
c        write ( 99, * ) time, c(1:6)
c     end if
...
!
     ...
     &  +c(5)*(c(4)*c(6)-c(2)*c(5))
      if(vj.gt.1.d-30) then
         vj=dsqrt(vj) ! vj is Jacobian of F
      else
...
!
      ...
      cb=(c(1)*beta(1)+c(2)*beta(2)+c(3)*beta(3)+
     &     2.d0*(c(4)*beta(4)+c(5)*beta(5)+c(6)*beta(6)))/3.d0
      do i = 1, 6
         devbeta(i) = beta(i) - cb * ci(i)
      end do

      

!     mbstress = 1.0/3.0 * ( beta(1) + beta(2) + beta(3) )
!     beta(1) = beta(1) - mbstress
!     beta(2) = beta(2) - mbstress
!     beta(3) = beta(3) - mbstress

!     stril = G J^(-2/3) DEV [ C^p ]
      do i=1,6
         stril(i)=um*cplb(i)-devbeta(i) 
      enddo
...
!     mstress = 1.0/3.0 * ( stril(1) + stril(2) + stril(3) )
!     stril(1) = stril(1) - mstress
!     stril(2) = stril(2) - mstress
!     stril(3) = stril(3) - mstress


c     if ( iel == 20 .and. iint == 1 ) then
c        write ( 0, * ) 'stril calculated to be'
c        write ( 0, 10 ) um
c        write ( 0, 20 ) cplb(1:6)
c        write ( 0, 20 ) stril(1:6)
c        write ( 0, 20 ) beta(1:6)
c        !pause 1
c     end if
...
c     if ( iel == 20 .and. iint == 1 ) then
c        write ( 599, * ) 'sc(1,1), sc(2,2), sc(3,3) = ',
c    &        sc(1,1), sc(2,2), sc(3,3)
c     end if

!
c     mstress = 1.0/3.0 * ( sc(1,1) + sc(2,2) + sc(3,3) )
c     sc(1,1) = sc(1,1) - mstress
c     sc(2,2) = sc(2,2) - mstress
c     sc(3,3) = sc(3,3) - mstress
      ...
      if(niso.ne.0) then
         ...
         do i = 1, 8
            if ( mod(i,4) == 1 .or. mod(i,4)==2 ) then
               xiso_counter = xiso_counter + 1
               params(i) = xiso(xiso_counter)
            else if ( mod(i,4) == 3 .or. mod(i,4) == 0 ) then
               yiso_counter = yiso_counter + 1
               params(i) = yiso(yiso_counter)/vj
            end if
         end do
      end if
...
!      if(1.eq.1) then ! always use uhardening
         call uhardening(amat,iel,iint,t1l,epini,ep,dtime,
     &        fiso,dfiso,fkin,dfkin,params,8,3)
         fiso=fiso*vj
!        dfiso=dfiso*vj
      else
         if(niso.ne.0) then
...
c     if ( iel == 20 .and. iint == 1 ) then
c        write ( 0, * ) 'ftrial caculated to be'
c        write ( 0, * ) ftrial, fiso, dxitril
c     end if
      if((ftrial.le.1.d-10).or.(ielas.eq.1).or.(ielastic.eq.1)
     &     .or.(dtime.lt.1.d-30)) then

c        if ( iel == 20 .and. iint == 1 ) then
c           write ( 0, * ) " no plastic deformation "
c        end if
!
...
!
         stre(1)=c8*ci(1)+stril(1) + devbeta(1) - beta(1) 
         stre(2)=c8*ci(2)+stril(2) + devbeta(2) - beta(2) 
         stre(3)=c8*ci(3)+stril(3) + devbeta(3) - beta(3)
         stre(4)=c8*ci(4)+stril(4) + devbeta(4) - beta(4)
         stre(5)=c8*ci(5)+stril(5) + devbeta(5) - beta(5)
         stre(6)=c8*ci(6)+stril(6) + devbeta(6) - beta(6)
!
...
c        open ( unit = 1200, file = 'debug1200.txt', position='append')
c        write ( 1200, * ) iel, iint, stre(1:6)
c        call flush(1200)
c        close(1200)

         
c        if ( iel == 20 .and. iint == 1 ) then
c           write ( 0, 20 ) stre(1:6)
            !pause 1
c        end if
...
!                 dfkin=dfkin*vj
                  ...
                  ! calculate iso hardening
                  if(niso.ne.0) then ! isotropic is active
                     call ident(xiso,ep,niso,id)
                     if(id.eq.0) then
                        fiso=yiso(1)
                     elseif(id.eq.niso) then
     ...
     &                        (xiso(id+1)-xiso(id))
                  ...
                  elseif(nkin.ne.0) then ! kinematic is active
                     fiso=ykin(1)
                  else ! nothing is active
                     fiso=0.d0
                  endif
!
                  ! calculate kin hardening
                  if(nkin.ne.0) then
                     call ident(xkin,ep,nkin,id)
                     if(id.eq.0) then
                        fkin=ykin(1)
                     elseif(id.eq.nkin) then
                        ...
                        dfkin=(ykin(id+1)-ykin(id))/    ! this is the code for calculating dfkin
     &                        (xkin(id+1)-xkin(id))
                        fkin=ykin(id)+dfkin*(ep-xkin(id))
                     endif
                  elseif(niso.ne.0) then
...
!
      ...
      c7=c6*dfkin*vj23
      do i=1,6
         stbl(i)=stbl(i)+c7*xitril(i) ! Ziegler ?
      enddo
!
!        updating the stress
!        vj: Jacobian of the total deformation gradient
!
      c8=xk*(vj2-1.d0)/2.d0 ! xk: bulk modulus
!
      do i=1,6
         stre(i) = c8*ci(i) + stril(i) - c5*xitril(i)
     1        + devbeta(i) - beta(i)
!        stre(i)=c8*ci(i)+stril(i)-c5*xitril(i)
      enddo
      
!     stre(1) = stre(1) - mbstress
!     stre(2) = stre(2) - mbstress
!     stre(3) = stre(3) - mbstress

c     if ( iel == 20 .and. iint == 1 ) then
c        write ( 199, * ) time, c8 , vj2, xk
c     end if
!
