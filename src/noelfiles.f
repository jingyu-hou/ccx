...
!
     ...
     &'*WARNING reading *NODE/EL/CONTACT FILE: SEN only makes'
            ...
            elseif(textpart(ii)(1:4).eq.'RDP ') then
               if((nmethod.eq.2).or.(nmethod.eq.3)) then
                  write(*,*) 
     &'*WARNING reading *NODE/EL/CONTACT FILE: selection of RDP'
                  write(*,*) '         does not make sense for a'
                  write(*,*) '         frequency or bucking calculation'
               else
                  filab(48)(1:4)='RDP '
                  filab(48)(6:6)=elemsys
                  filab(48)(7:87)=noset
               endif
            elseif(textpart(ii)(1:4).eq.'DRX ') then
               if((nmethod.eq.2).or.(nmethod.eq.3)) then
                  write(*,*) 
     &'*WARNING reading *NODE/EL/CONTACT FILE: selection of DRX'
                  write(*,*) '         does not make sense for a'
                  write(*,*) '         frequency or bucking calculation'
               else
                  filab(49)(1:4)='DRX '
            ...
            elseif(textpart(ii)(1:4).eq.'DMG ') then
               if((nmethod.eq.2).or.(nmethod.eq.3)) then
                  write(*,*) 
     &'*WARNING reading *NODE/EL/CONTACT FILE: selection of DMG'
                  write(*,*) '         does not make sense for a'
                  write(*,*) '         frequency or bucking calculation'
               else
                  filab(50)(1:4)='DMG '
