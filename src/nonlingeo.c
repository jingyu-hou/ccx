...
/*  if((*nstate_!=0)&&(*mortar==1)) {
  ...
  if((*nstate_!=0)&&(*mortar==1)) {
      NNEW(xstateini,double,*nstate_*mi[0]*(*ne+*nintpoint));
      for(k=0;k<*nstate_*mi[0]*(*ne+*nintpoint);++k) {
              xstateini[k]=xstate[k];
      }
  }
  ...
  if((*nmethod==4)&&(*ithermal!=2)&&(icfd!=1)){
      ...
      results(co,nk,kon,ipkon,lakon,ne,vold,stn,inum,stx,
	      ...
	      islavsurf,ielprop,prop,energyini,energy,&kscale,iponoel,
              inoel,nener,orname,network,ipobody,xbodyact,ibody,mpcon,nmpcon,
	      nmpmat_,pphase,cphase,phaseother,nphase,phase_inf,rdpcon,gscon);
	...
	mafillsmmain(co,nk,kon,ipkon,lakon,ne,nodeboun,ndirboun,xbounact,nboun,
		  ...
		  t0,t1act,ithermal,prestr,iprestr,vold,vini,iperturb,sti,
...
/*		  if(nasym!=0){
      ...
      results(co,nk,kon,ipkon,lakon,ne,v,stn,inum,stx,
	      ...
	      nmpcon,nmpmat_,pphase,cphase,phaseother,nphase,phase_inf,rdpcon,
	      gscon);
      SFREE(inum);
/*
      fprintf ( pFile, "\n %s \n","v matrix");
...
*/
	  ...
	  results(co,nk,kon,ipkon,lakon,ne,v,stn,inum,stx,
	      ...
	      inoel,nener,orname,network,ipobody,xbodyact,ibody,mpcon,nmpcon,nmpmat_,
              pphase,cphase,phaseother,nphase,phase_inf,rdpcon,gscon);
	  SFREE(inum);
      }
...
//	  f[k]=fini[k];
    ...
    if((jout[0]==jprint)&&(icutb==0)){
      ...
      if((strcmp1(&filab[609],"SDV ")==0)||
         (strcmp1(&filab[4089],"RDP ")==0)||
         (strcmp1(&filab[4176],"DRX ")==0)||
	 (strcmp1(&filab[4263],"DMG ")==0))
