...
void plasticwork ( double * xstate,  int * nstate, 
...
//		     3, debug_dbles, debug_dbles_anno, 1000 ) ;
  ...
  //exit(0);
  /*
  largest_ptr_dtemp_el = 0.0 ;
  ie_largest_ptr_dtemp_el = 0 ;
  for (ie = 0; ie < *ne; ie ++ ) {
    if ( ptr_dtemp_el[ie] > largest_ptr_dtemp_el ) {
      largest_ptr_dtemp_el = ptr_dtemp_el[ie];
      ie_largest_ptr_dtemp_el = ie ;
    }
  } printf( "largest_ptr_dtemp_el = %f\n", largest_ptr_dtemp_el );
  printf( "ie_largest_ptr_dtemp_el = %d\n", ie_largest_ptr_dtemp_el );
  */
