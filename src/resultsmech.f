...
!
     ...
     &  list,ilist,mpcon,nmpcon,nmpmat_,rdpcon,gscon)
!
...
!
     ...
     &  istrainfree,nlgeom_undo,list,ilist(*),m,nmpcon(2,*),nmpmat_
!
     ...
     &  pslavsurf(3,*),pmastsurf(6,*),mpcon(2,nmpmat_,*),
     &  mpconloc(800),i0(3,3),xokl(3,3),voj,vj0,voj0,
     &  vij0,ej,eoj,eij,kp0,k0p,n0,np,shpc(4,26),xsjc,pp,p0,p,
     &  fnb(24,1),b(6,24),sigma(6,1),vr,rdpcon(10,3,*),gscon(30,5,*),
     &  rdploc(10,3),gsloc(30,5),vinil(0:mi(2),26),til,tol
!
...
!
     ...
     &            (lakonl(7:7).eq.'2')) then
               ...
               elseif(ithermal(1).ge.2) then
                  t0l=(t0(konl(1))+t0(konl(2)))/2.d0
                  til=(vini(0,konl(1))+vini(0,konl(2)))/2.d0
                  tol=(vold(0,konl(1))+vold(0,konl(2)))/2.d0
                  t1l=(v(0,konl(1))+v(0,konl(2)))/2.d0
               endif
            endif
!
...
!
cccccccccccccccccccccc bbar cycle 1 start cccccccccccccccccccccccccccccc
!
         if(lakonl(1:5).eq.'C3D8 ') then
...
!
                  ...
                  elseif(ithermal(1).ge.2) then
                     do i1=1,8
                        t0l=t0l+t0(konl(i1))/8.d0
                        til=til+vini(0,konl(i1))/8.d0
                        tol=tol+vold(0,konl(i1))/8.d0
               ...
               do m1=1,nope
                  do m2=1,3
                     do m3=1,3
                        ...
                        vokl(m2,m3)=vokl(m2,m3)+shp(m3,m1)*voldl(m2,m1)
                        if(kode.le.-70) then
                          vikl(m2,m3)=vikl(m2,m3)
     &                      +shp(m3,m1)*vinil(m2,m1)
                        endif
                     enddo
                  enddo
               enddo
!
               do m1=1,3
                  do m2=1,3
                     xkl(m1,m2)=vkl(m1,m2)+i0(m1,m2)
                  enddo
               enddo
               
               if(ithermal(1).ne.0) then
                  call calceth(alcon,nalcon,imat,iorien,pgauss,
     &               orab,ntmat_,alzero,t0l,t1l,eth)
                  call calcmechstrain(xkl,eth)
               endif
               ...
               if(ithermal(1).ne.0) then
                  call calceth(alcon,nalcon,imat,iorien,pgauss,
     &               orab,ntmat_,alzero,t0l,tol,eth)
                  call calcmechstrain(xokl,eth)
               endif
               ...
               if(kode.le.-70) then
                  ...
                  if(ithermal(1).ne.0) then
                     call calceth(alcon,nalcon,imat,iorien,pgauss,
     &                  orab,ntmat_,alzero,t0l,til,eth)
                     call calcmechstrain(xikl,eth)
                  endif
               endif
!
...
!
     ...
     &           +xkl(1,3)*(xkl(2,1)*xkl(3,2)-xkl(2,2)*xkl(3,1))
               voj=xokl(1,1)*(xokl(2,2)*xokl(3,3)-xokl(2,3)*xokl(3,2))
     &            -xokl(1,2)*(xokl(2,1)*xokl(3,3)-xokl(2,3)*xokl(3,1))
     &            +xokl(1,3)*(xokl(2,1)*xokl(3,2)-xokl(2,2)*xokl(3,1))
               if(kode.le.-70) then
                 vij=xikl(1,1)*(xikl(2,2)*xikl(3,3)-xikl(2,3)*xikl(3,2))
     &              -xikl(1,2)*(xikl(2,1)*xikl(3,3)-xikl(2,3)*xikl(3,1))
     &              +xikl(1,3)*(xikl(2,1)*xikl(3,2)-xikl(2,2)*xikl(3,1))
               endif
               
               kp0=kp0+n0*np*weight*xsj
               
               ej =ej+np*vj*weight*xsj
               eoj=eoj+np*voj*weight*xsj
               if(kode.le.-70) then
                 eij=eij+np*vij*weight*xsj
               endif
            enddo
...
cccccccccccccccccccccc bbar cycle 1 end   cccccccccccccccccccccccccccccc

cccccccccccccccccccccc bbar cycle 2 start cccccccccccccccccccccccccccccc
...
!
     ...
     &               (lakonl(4:5).eq.'8I')) then
                     do i1=1,8
                        t0l=t0l+t0(konl(i1))/8.d0
c                        t1l=t1l+t1(konl(i1))/8.d0
                        t1l=t1l+v(0,konl(i1))/8.d0
                     enddo
                  elseif(lakonl(4:6).eq.'20 ') then
                     nopered=20
c                     call lintemp(t0,t1,konl,nopered,jj,t0l,t1l)
                     call lintemp_th(t0,v,konl,nopered,jj,t0l,t1l,mi)
                  elseif(lakonl(4:6).eq.'10T') then
                     call linscal10(t0,konl,t0l,null,shp)
c                     call linscal10(t1,konl,t1l,null,shp)
                     call linscal10(v,konl,t1l,mi(2),shp)
                  else
                     do i1=1,nope
                        t0l=t0l+shp(4,i1)*t0(konl(i1))
c                        t1l=t1l+shp(4,i1)*t1(konl(i1))
     ...
     &               (lakonl(4:5).eq.'8I')) then
                  ...
                  elseif(lakonl(4:6).eq.'20 ') then
                     nopered=20
                     call lintemp_th(t0,vini,konl,nopered,jj,t0l,til,mi)
                     call lintemp_th(t0,vold,konl,nopered,jj,t0l,tol,mi)
                     call lintemp_th(t0,v,konl,nopered,jj,t0l,t1l,mi)
                  elseif(lakonl(4:6).eq.'10T') then
                     call linscal10(t0,konl,t0l,null,shp)
                     call linscal10(vini,konl,til,mi(2),shp)
                     call linscal10(vold,konl,tol,mi(2),shp)
                     call linscal10(v,konl,t1l,mi(2),shp)
                  else
                     do i1=1,nope
                        t0l=t0l+shp(4,i1)*t0(konl(i1))
                        til=til+shp(4,i1)*vini(0,konl(i1))
                        tol=tol+shp(4,i1)*vold(0,konl(i1))
                        t1l=t1l+shp(4,i1)*v(0,konl(i1))
                     enddo
                  endif
               endif
...
!
     ...
     &        +xkl(1,3)*(xkl(2,1)*xkl(3,2)-xkl(2,2)*xkl(3,1))
            ...
            if(lakonl(1:5).eq.'C3D8 ') then
               vr=(abs(vj0/vj))**(1.d0/3.d0)*sign(1.d0,(vj0/vj))
               xkl=vr*xkl
               vj=vj0
            endif
...
!
     ...
     &                       shp(m3,m1)*voldl(m2,m1)
               ...
               enddo
!
!           culating the deformation gradient
!
               xokl=vokl+i0
     ...
     &            +xokl(1,3)*(xokl(2,1)*xokl(3,2)-xokl(2,2)*xokl(3,1))
               ...
               if(lakonl(1:5).eq.'C3D8 ') then
                  vr=(abs(voj0/voj))**(1.d0/3.d0)*sign(1.d0,(voj0/voj))
                  xokl=vr*xokl
                  voj=voj0
               endif
...
!
            if((kode.eq.-50).or.(kode.le.-70)) then
!               
!
...
!
     ...
     &            +xikl(1,3)*(xikl(2,1)*xikl(3,2)-xikl(2,2)*xikl(3,1))
               ...
               if(lakonl(1:5).eq.'C3D8 ') then
                  vr=(abs(vij0/vij))**(1.d0/3.d0)*sign(1.d0,(vij0/vij))
                  xikl=vr*xikl
                  vij=vij0
               endif
               vikl=xikl-i0
...
!     
     ...
     &           xstiff,ncmat_,xstate,nstate_,mpcon,nmpcon,nmpmat_,
     &           mpconloc,rdpcon,gscon,rdploc,gsloc)
!
...
!
     ...
     &           amat,t0l,t1l,dtime,time,ttime,i,jj,nstate_,mi(1),
     &           iorien,pgauss,orab,eloc,mattyp,qa(3),istep,iinc,
     &           ipkon,nmethod,iperturb,qa(4),nlgeom_undo,mpconloc,
     &           rdploc,gsloc)
!
...
!           for user materials (kode<=-70) the mechanical strain has to
!           be updated at the end of each increment (also if no output
!           is requested), since it is input to the umat routine
!
            if((iout.gt.0).or.(iout.eq.-2).or.(kode.le.-70).or.
     &         ((nmethod.eq.4).and.
     &          ((iperturb(1).gt.1).and.(nlgeom_undo.eq.0)).and.
     &          (ithermal(1).le.1))) then
               if(nener.eq.1) then
                  ener(jj,i)=enerini(jj,i)+
     &                 ((eloc(1)-emeini(1,jj,i))*
     &                  (stre(1)+stiini(1,jj,i))+
     &                  (eloc(2)-emeini(2,jj,i))*
     &                  (stre(2)+stiini(2,jj,i))+
     &                  (eloc(3)-emeini(3,jj,i))*
     &                  (stre(3)+stiini(3,jj,i)))/2.d0+
     &         (eloc(4)-emeini(4,jj,i))*(stre(4)+stiini(4,jj,i))+
     &         (eloc(5)-emeini(5,jj,i))*(stre(5)+stiini(5,jj,i))+
     &         (eloc(6)-emeini(6,jj,i))*(stre(6)+stiini(6,jj,i))
               endif
               eme(1,jj,i)=eloc(1)
               eme(2,jj,i)=eloc(2)
               eme(3,jj,i)=eloc(3)
...
!
            if((iout.gt.0).or.(iout.eq.-2).or.(kode.le.-70)) then
!
               eei(1,jj,i)=eloc(1)
               eei(2,jj,i)=eloc(2)
               eei(3,jj,i)=eloc(3)
...
!               if((kode.ne.-50).and.(kode.gt.-70)) then
!c     Bernhardi start
...
!
            if(lakonl(1:5).eq.'C3D8 ') then
               ckl=matmul(xkl,matmul(skl,transpose(xkl)))/vj
               pp=pp+n0*(ckl(1,1)+ckl(2,2)+ckl(3,3))/3.d0*weight*xsj
            endif

         enddo
cccccccccccccccccccccc bbar cycle 2 end   cccccccccccccccccccccccccccccc

cccccccccccccccccccccc bbar cycle 3 start cccccccccccccccccccccccccccccc
         if((calcul_fn.eq.1).and.(lakonl(1:5).eq.'C3D8 '))then
...
!
     ...
     &           +xkl(1,3)*(xkl(2,1)*xkl(3,2)-xkl(2,2)*xkl(3,1))
               ...
               if((calcul_cauchy.ne.1).or.(nlgeom_undo.ne.0)) then
                  skl=matmul(xkl,matmul(skl,transpose(xkl)))/vj0
               endif
                  
               p=(skl(1,1)+skl(2,2)+skl(3,3))/3.d0
               skl=skl+(vj/vj0*p0-p)*i0
               ...
               sigma(6,1)=skl(3,2)
!               
               call bmatrix(xl,vl,mi,nope,jj,b,shpc,xsjc)
!               
               fnb=matmul(transpose(b),sigma)*vj0*weight*xsj
...
cccccccccccccccccccccc bbar cycle 3 end   cccccccccccccccccccccccccccccc
...
!         if (m.eq.1) then
!            open(2222, file='resultsmech.txt')
!            write(2222,*)"iinc", iinc , "fn", vj0
!            do m1=1,8
!               write(2222,"(3e15.6)")fn(1:3,m1)
!            enddo
!         endif
