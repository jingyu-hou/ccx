...
!
     ...
     &  xl(3,26),xlini(3,26),vl(0:mi(2),26),elcon(0:ncmat_,ntmat_,*),
     ...
     &  xbody(7,*),mpcon(2,nmpmat_,*),gscon(30,5,*)
...
!
         do j=1,nope
            ...
            do k=1,3
               xl(k,j)=co(k,konl(j))
               xlini(k,j)=co(k,konl(j))+vini(k,konl(j))
               vl(k,j)=v(k,konl(j))
            enddo
            vl(0,j)=v(0,konl(j))
         enddo
...
!
            if(nope.eq.20) then
               if(lakon(i)(7:7).eq.'A') then
                  call shape20h_ax(xi,et,ze,xlini,xsj,shp,iflag)
               elseif((lakon(i)(7:7).eq.'E').or.
     &                (lakon(i)(7:7).eq.'S')) then
                  call shape20h_pl(xi,et,ze,xlini,xsj,shp,iflag)
               else
                  call shape20h(xi,et,ze,xlini,xsj,shp,iflag)
               endif
            elseif(nope.eq.8) then
               call shape8h(xi,et,ze,xlini,xsj,shp,iflag)
            elseif(nope.eq.10) then
               call shape10tet(xi,et,ze,xlini,xsj,shp,iflag)
            elseif(nope.eq.4) then
               call shape4tet(xi,et,ze,xlini,xsj,shp,iflag)
            elseif(nope.eq.15) then
               call shape15w(xi,et,ze,xlini,xsj,shp,iflag)
            else
               call shape6w(xi,et,ze,xlini,xsj,shp,iflag)
            endif
            c1=xsj*weight
!
