...
void sensitivity(double *co, ITG *nk, ITG **konp, ITG **ipkonp, char **lakonp,
  ...
  if(icoordinate==1){
      ...
      /* if the sensitivity calculation is used in a optimization script
         ...
         1. a call to WeICME to define the sensitivities
...
/*	  for(i=0;i<nev;i++){
  ...
  for(iev=0;iev<nev;iev++){
	 ...
	 (calculated in results_se and needed in mafillsmse */
	  ...
	  &ieigenfrequency,fint,&ishapeenergy,mpcon,nmpcon,nmpmat_,
	  rdpcon,gscon);
	  ...
	  iponoel,inoel,nodedesiinv,dgdxglob,df2,dgdxdy,nkon,mpcon,nmpcon,
