...
static void ssolve_permuteout(DenseMtx *mtxX, IV *newToOldIV, FILE *msgFile)
{
 ...
 void factor(struct factorinfo *pfi, InpMtx *mtxA, int size, FILE *msgFile,
             int *symmetryflagi4,int *singularflag)
{
	Graph *graph;
	IVL *symbfacIVL;
	Chv *rootchv;
	 ...
	 */
	{
		...
		if (rootchv != NULL) {
			fprintf(pfi->msgFile, "\n\n matrix found to be singular 215\n");
                        *singularflag=1;
			return;
//			exit(-1);
		}
               ...
               int *singularflag)
{
	 ...
	 */
	{
		...
		if (rootchv != NULL) {
			fprintf(pfi->msgFile, "\n\n matrix found to be singular 371\n");
                        *singularflag=1;
			return;
//			exit(-1);
		}
...
void spooles_factor(double *ad, double *au,  double *adb, double *aub, 
	     ...
	     ITG *symmetryflag, ITG *inputformat, ITG *nzs3,ITG *singularflag)
{
	int size = *neq;
...
/*	if(*neq==0) return;*/
	...
	/*
	 * Create the InpMtx object from the WeICME matrix
	 *      representation
	 */
...
//			printf("row=%d,col=%d,value=%e\n",col,row,au[ipo+*nzs]);
	...
	if (num_cpus > 1) {
		 ...
		 * locking overhead
		 */
		factor_MT(&pfi, mtxA, size, msgFile,&symmetryflagi4,singularflag);
	} else {
		factor(&pfi, mtxA, size, msgFile,&symmetryflagi4,singularflag);
	}
#else
...
//	printf(" Using 1 cpu for spooles.\n\n");
	...
	/* convert the result back to WeICME representation */
	{
		int i;
		for (i = 0; i < size; i++) {
...
/*	if(*neq==0) return;*/
	...
	if (num_cpus > 1) {
		 ...
		 * locking overhead
		 */
		factor_MT(&pfj, mtxA, size, msgFilf,&symmetryflagi4,singularflag);
	} else {
		factor(&pfj, mtxA, size, msgFilf,&symmetryflagi4,singularflag);
	}
#else
	printf(" Using 1 cpu for spooles.\n\n");
	factor(&pfj, mtxA, size, msgFilf,&symmetryflagi4,singularflag);
...
/** 
 * spooles: Main interface between WeICME and spooles:
 *
             ...
             ITG *symmetryflag, ITG *inputformat, ITG *nzs3,ITG *singularflag)
{
  ...
  spooles_factor(ad,au,adb,aub,sigma,icol,irow,neq,nzs,symmetryflag,
		 inputformat,nzs3,singularflag);
  
  if(*singularflag!=0){return;}
