...
void steadystate(double **cop, ITG *nk, ITG **konp, ITG **ipkonp, char **lakonp, ITG *ne, 
	       ...
	       ITG *ndamp,double *dacon,double *mpcon, ITG *nmpcon,ITG *nmpmat_,
               double*pphase,double *cphase,double *phaseother, ITG *nphase,
	       ITG *phase_inf,double *rdpcon,double *gscon){
  ...
  ITG nev,i,j,k, *inum=NULL,*ipobody=NULL,inewton=0,nsectors,im,
    ...
    *islavnode=NULL,*nslavnode=NULL,*islavsurf=NULL,iit=-1,
    network=0,kscale=0,nmethodact=1,singularflag=NULL;
...
//izdofNNEW(//      izdof,ITG,1);
	...
	&nherm,xmr,xmi,typeboun,ielprop,prop,orname,mpcon,nmpcon,nmpmat_,
    pphase,cphase,phaseother,nphase,phase_inf);
  ...
  if(dashpot){
      ...
      if(nherm!=1){
	  printf("ERROR in steadystate: dashpots cannot be combined with non-Hermitian systems (in the present version of WeICME)\n");
	  FORTRAN(stop,());
      }
...
/*	      FORTRAN(mafilldm,(co,nk,kon,ipkon,lakon,ne,nodeboun,
		  ...
		  &freq[l],ndamp,dacon,mpcon,nmpcon,nmpmat_,rdpcon,gscon);
	  ...
	  if(iprescribedboundary){
	      ...
	      results(co,nk,kon,ipkon,lakon,ne,vr,stnr,inum,
		      ...
		      clearini,islavsurf,ielprop,prop,energyini,energy,&iit,
                      ...
                      ibody,mpcon,nmpcon,nmpmat_,pphase,cphase,phaseother,nphase,
		      phase_inf,rdpcon,gscon);
